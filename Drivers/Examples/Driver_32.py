...
class Driver_32(DriverBase):
    ...
    def _think(self, car_sensor: CarSensor, track_sensor: LineSensor):
        ...
        start_pos = self.storage.get("start_pos", [0, 0])
        speed_y, speed_x = 0, 0
        pos_y, pos_x = car_sensor.get_position()
        is_first_run = self.storage.get("first_run", True)
        iteration = self.storage.get("iteration",0)
        if iteration == 0:
            self.storage.set("start_pos", [pos_y,pos_x])
            print("first run")
            self.storage.set("iteration", 1)
            map_size = track_sensor.get_track_size()
            self.storage.set("map_size",map_size)
            map_size = self.storage.get("map_size",map_size)
            start_direction = track_sensor.get_start_direction()
            self.storage.set("start_direction", start_direction)
            track_point1 = [0,0]
            track_point2 = [0,0]
            if start_direction == [0, -1]:
                track_point1 = track_sensor.is_path_homogeneous((pos_y,pos_x), (0,pos_x))
                track_point2 = track_sensor.is_path_homogeneous((pos_y,pos_x), (map_size[0],pos_x))
                track_width = abs(track_point1.last_same_as_start[0] - track_point2.last_same_as_start[0])
            elif start_direction == [0,1]:
                track_point1 = track_sensor.is_path_homogeneous((pos_y,pos_x), (map_size[0],pos_x))
                track_point2 = track_sensor.is_path_homogeneous((pos_y,pos_x), (0,pos_x))
                track_width = abs(track_point1.last_same_as_start[1] - track_point2.last_same_as_start[1])
            elif start_direction == [1, 0]:
                track_point1 = track_sensor.is_path_homogeneous((pos_y, pos_x), (pos_y, map_size[1]))
                track_point2 = track_sensor.is_path_homogeneous((pos_y, pos_x), (pos_y, 0))
                track_width = abs(track_point1.last_same_as_start[1] - track_point2.last_same_as_start[1])
            elif start_direction == [-1, 0]:
                track_point1 = track_sensor.is_path_homogeneous((pos_y, pos_x), (pos_y, 0))
                track_point2 = track_sensor.is_path_homogeneous((pos_y, pos_x), (pos_y, map_size[1]))
                track_width = abs(track_point1.last_same_as_start[1] - track_point2.last_same_as_start[1])
            print("hello")
            self.storage.set("track_width", track_width)
            self.storage.set("track_point1", track_point1.last_same_as_start)
            self.storage.set("track_point2", track_point2.last_same_as_start)
        ...
        if iteration == 1:
            ...
            print(self.get_corner(start_direction, 0, self.storage.get("track_point1",0), True, track_sensor, map_size, track_width, self.storage.get("track_point2",[0,0])))
    ...
    def get_corner(self, direction, prev_corner_in_dir, apex_or_edge, edge, track_sensor, map_size, track_width, track_point2):

        if edge:

            line1 = track_sensor.is_path_homogeneous((apex_or_edge[0]+direction[1], apex_or_edge[1]+direction[0]),(abs(map_size[0] * direction[0]), abs(map_size[1] * direction[1])))
            line2_start = track_point2[0]+(-direction[1]), track_point2[1]+(-direction[0])
